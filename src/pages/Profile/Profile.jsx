...
export default function Profile() {
  ...
  const Row = ({ label, value, mono }) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 0', borderBottom: '1px solid var(--border)' }}>
      ...
      <span style={{ fontSize: 12, fontFamily: mono ? 'var(--font-mono)' : 'inherit', color: 'var(--text)' }}>{value || '—'}</span>
    </div>
  );
        ...
        <div className="card">
          ...
          <div className="card-body">
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16, paddingBottom: 14, borderBottom: '1px solid var(--border)' }}>
              <div style={{ width: 48, height: 48, background: 'var(--accent)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, fontWeight: 900, color: '#000' }}>
                {data.dfsp_id?.[0]?.toUpperCase()}
              </div>
              <div>
                <div style={{ fontWeight: 700, fontSize: 15 }}>{data.dfsp_name}</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--accent)' }}>{data.dfsp_id}</div>
              </div>
            </div>
          ...
          <div className="card-body">
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 16, paddingBottom: 14, borderBottom: '1px solid var(--border)' }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text-muted)' }}>{data.username}</div>
              </div>
            </div>
            <Row label="Email"      value={data.email} />
            <Row label="Role"       value={data.role}  mono />
            <Row label="Last Login" value={data.last_login ? new Date(data.last_login).toLocaleString() : 'N/A'} />
          </div>
        </div>
      </div>
    </div>
